...
export function calculateRobotUtilization(
  ...
  const capacity24x7 = robots * ROBOT_CAPACITY.HOURS_24X7;
  ...
  const utilizationRisk24x7 = calculateRobotUtilizationRisk(utilization24x7);

  const { risk: expiryRisk, daysUntil } = calculateExpiryRisk(expiryDate);

  const overallRisk = getWorstRisk([utilizationRisk24x7, expiryRisk]);
